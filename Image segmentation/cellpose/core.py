...
class UnetModel():
    def __init__(self, gpu=False, pretrained_model=False,
        ...
        self.nchan = nchan
        self.nbase = [nchan, 32, 64, 128, 256,512]
...
#         slc = [slice(0, self.nclasses) for n in range(imgs.ndim)] # changed from imgs.shape[n]+1 for first slice size 
    ...
    def _train_step(self, x, lbl):
        ...
        train_loss *= len(x)
        return train_loss
    def comput_ap(self,lbl,y):
        map_list=[]
        for i in range(len(lbl)):
            print(len(lbl),y[i].size(),print(np.shape(lbl)))
            ap = metrics.average_precision(np.array(lbl[i], dtype=np.int16), np.array(y[i].cpu(),dtype=np.int16))[0]
            map_list.append(ap)
    ...
    def _test_eval(self, x, lbl):
        ...
        with torch.no_grad():
            y, style = self.net(X)
            del X
            # print('lbl.size(),y.size():',np.shape(lbl))
            # print("y.size():",y.size())
            loss = self.loss_fn(lbl,y)
            map=0#=self.comput_ap(lbl, y)
            test_loss = loss.item()
            test_loss *= len(x)
            # for i in range(len(lbl)):
            #     # print("lbl.size()", np.shape(lbl1))
            #     print("lbl.size(),y.size():",np.shape(lbl[i]),np.shape(np.array(y[i].cpu())))
            #     print(lbl[i],np.array(y[i].cpu()))
            #     ap = metrics.average_precision(lbl[i], np.array(y[i].cpu()))[0]
            #     ap_list.append(ap[:, 0].mean())
            # print(f'>>> average precision at iou threshold 0.5 = {ap_list.mean():.3f}')
    ...
    def plot_data(self,train_loss,test_loss):
    ...
    def _train_net(self, train_data, train_labels, 
        ...
        self.batch_size = batch_size
        self._set_optimizer(learning_rate, momentum, weight_decay, SGD)
        self._set_criterion()
        ...
        print("################开始训练#########################")
        print("save_path:{} \n self.n_epochs:{}, "
              "self.batch_size:{},nclasses:{},"
              "learning_rate:{}".format(save_path,self.n_epochs,
                        self.batch_size,self.nclasses,learning_rate))
        ...
        for iepoch in range(self.n_epochs):
            ...
            print('Epoch %d, Time %4.1fs, Loss %2.4f, LR %2.6f'%
                            (iepoch, time.time()-tic, lavg, lr_))
            lr_ = learning_rate * (1.0 - iepoch / self.n_epochs) ** 0.9
            if iepoch%5==0 or iepoch==5:
                lavg = lavg / nsum
                if test_data is not None:
                    lavgt, nsum = 0., 0
                    avg_ap=0
                    ...
                    for ibatch in range(0,len(test_data),batch_size):
                        ...
                        test_loss,map = self._test_eval(imgi, lbl)
                        avg_ap+=map
                        lavgt += test_loss
                        nsum += len(imgi)
                    print('Epoch %d, Time %4.1fs, Loss %2.4f, Loss Test %2.4f,map Test %2.4f, LR %2.4f'%
                            (iepoch, time.time()-tic, lavg, lavgt/nsum,avg_ap/nsum, lr_))
                    ...
                    # train_loss_list.append(lavg)
                    # test_loss_list.append(lavgt/nsum)
                else:
        ...
        # chan = 2
        # chan2 = 1
        # diam_labels = self.net.diam_labels#.copy()
        # masks = self.net.eval(test_data,
        #                    channels=[chan, chan2],
        #                    diameter=diam_labels)[0]
        # ap_list = []
        # for i in range(len(test_labels)):
        #     ap = metrics.average_precision(np.array(test_labels[i], dtype=np.int16), masks[i])[0]
        #     ap_list.append(ap[0])
