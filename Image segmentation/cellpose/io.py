...
def logger_setup():
    ...
    log_file = cp_dir+'run_1_test.log'
    try:
        log_file.unlink()
    except:
...
def imread(filename):
    ...
    if ext== '.tif' or ext=='.tiff':
        ...
        try:
            ...
            if h<224:
                img=cv2.resize(img,(224,w),interpolation=cv2.INTER_NEAREST)
            if w<224:
                img=cv2.resize(img,(h,224))
            if img.ndim > 2:
                img = img[..., [2,1,0]]
            return img
...
def get_label_files(image_names, mask_filter, imf=None):
    ...
    #print("2345678:",label_names[0] + mask_filter + '.png')
