...
from model import Discriminator, TruncatedVGG19, RDN
from CustomDataset import SRDataset
...
def train_method():
    # 数据集参数
    data_folder = 'data/DIV2K_train_HR/'  # 数据存放路径
    crop_size = 128  # 高分辨率图像裁剪尺寸
    ...
    generator = RDN(scale_factor=scaling_factor,
                    num_channels=3,
                    num_features=64,
                    growth_rate=64,
                    num_blocks=16,
    ...
    texture_loss = texture_loss.to(device)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for i, (lr_imgs, hr_imgs) in enumerate(train_loader):
            ...
            text_loss = texture_loss(sr_imgs_in_vgg_space, hr_imgs_in_vgg_space)
            # 计算总的感知损失
            perceptual_loss = content_loss + beta * adversarial_loss + text_loss
        ...
        }, 'save_model/checkpoint_srRDB3.pth')
    ...
    plt.xlabel('epoch')
    plt.plot(loss_list)
    # plt.legend(['Loss curve (Discriminator)'])
    plt.savefig('results/srgan_training loss.png', format='png')
    plt.show()
    val_epoch = pd.DataFrame(data=loss_list)
    val_epoch.to_csv('results/srgan_loss_per_epoch.csv', encoding='gbk')
